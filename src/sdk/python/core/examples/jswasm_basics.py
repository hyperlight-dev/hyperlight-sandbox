...
File I/O tests live in jswasm_filesystem_demo.py.
Network tests live in jswasm_network_demo.py.
"""
...
print("\n--- Test 1: Basic execution ---")
result = timed_run(sandbox, 'console.log("hello from JS wasm component!");', "first run (cold)")
print(f"stdout: {result.stdout!r}")
assert result.success, result.stderr
assert "hello from JS wasm component!" in result.stdout
...
assert "Hello, James!" in result.stdout

# Test 3: Snapshot/restore
print("\n--- Test 3: Snapshot/restore ---")
snap = sandbox.snapshot()
result = timed_run(
    sandbox, "globalThis.counter = 100; console.log('Set counter = ' + globalThis.counter);", "pre-restore"
)
print(f"Before restore: {result.stdout!r}")
...
print(f"After restore: {result.stdout!r}")
assert result.success
assert "state was rolled back" in result.stdout
