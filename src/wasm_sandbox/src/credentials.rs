...
//!
//! * `target`   — URL-prefix scope (e.g. `"https://api.example.com/"`).
//!   The outgoing-handler only injects the credential when
//!   the request URL starts with this prefix.
...
//!   string key; a future commit will support async
//!   resolution callbacks.
//!
//! The registry lives on [`crate::HostState`] behind
//! `Arc<Mutex<HashMap<String, CredentialEntry>>>` and is populated by
//! the host before the guest runs.  Guests bind a credential to a
//! specific outgoing request via the WIT `attach` function.
